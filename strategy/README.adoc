...
Uma implementação de uma solução sem utilizar o padrão Strategy normalmente é por meio de uma única classe, a qual chamaremos de `NaoEstrategista`. Não há nem ao menos um diagrama, pois a solução seria composta de fato apenas por esta classe. Tal classe então teria métodos como `T executarComportamentoA()`. Todas as implementações deste "Comportamento A" seriam incluídas no método citado.
...
Se por acaso em um lugar do sistema você precisa processar os arquivos e incluir os dados em um banco de dados e em outro você precisa processar e gerar PDFs com comprovantes de pagamentos ou enviar emails de notificação, em cada um desses locais você precisará deste bloco de _if's_.
...
Graças à Programação Funcional em diversas linguagens como Java 8+, JavaScript, PHP, Phython e outras, podemos simplificar este diagrama, e consequentemente a implementação, como mostra a figura a seguir.
