...
public abstract class AbstractCepService implements CepService {
    ...
    public Endereco buscaEndereco(final String cep) {
         ...
         */
        String json = target.path(buildPath(cep)).request().get(String.class);
        return jsonToEndereco(json);
    }

    /**
     * Converte um endereço representando em JSON para um objeto {@link Endereco} em Java.
     * Como cada subclasse desta classe abstrata representa um serviço de CEP diferente,
     * cada serviço pode retornar um JSON representando um objeto endereço mas
     * com nomes diferentes para os atributos.
     * Inclusive, alguns serviços podem ter campos que outros não tem.
     * Por isso, cada subclasse deve implementar este método para definir
     * como converter o JSON para um objeto {@link Endereco} padrão.
     *
     * @param json String representando o JSON do objeto endereço
     * @return um objeto {@link Endereco} criado a partir do JSON
     */
    protected abstract Endereco jsonToEndereco(String json);
